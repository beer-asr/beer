...
def main():
    ...
    parser.add_argument('-s', '--dlatent-dim', type=int, default=2,
                        help='dimension of the latent space')
    ...
    # Discriminant latent prior (Normal).
    dlatent_prior = beer.Normal.create(
        mean=torch.zeros(args.latent_dim),
        cov=torch.ones(args.latent_dim),
        cov_type='full',
        prior_strength=1e-3
    ...
    gsmset = beer.GSMSet.create(model, len(X), args.latent_dim,
                                args.dlatent_dim, latent_prior, dlatent_prior)
    gsm = beer.Mixture.create(gsmset)

    # Model instances.
    models, latent_posts = gsmset.new_models(len(X), cov_type='diagonal')
