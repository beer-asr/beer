...
echo "--> Train the unigram AUD system"
steps/aud.sh \
    --prior $prior \
...
echo "--> Decoding $db/$test dataset"
outdir=$expdir/$db/$subset/aud_${feaname}_${ngauss}g_${prior}/decode_perframe/$test
steps/decode.sh \
    --per-frame \
    --parallel-opts "-l mem_free=1G,ram_free=1G" \
    --parallel-njobs 30 \
    ...
    data/$db/$subset/$test \
    $expdir/$db/$subset/datasets/$feaname/${test}.pkl \
    $outdir
