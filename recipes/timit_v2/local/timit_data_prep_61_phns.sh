...
if [ $# -ne 3 ]; then
    echo "$0 /path/to/timit datadir conf_dir"
    exit 1;
fi
...
[ -f $conf_dir/phones.60-48-39.map ] || error_exit "$PROG: phones.60-48-39.map not found."
...
if $uppercased; then
    tr '[:lower:]' '[:upper:]' < $conf_dir/dev_spk.list > $tmpdir/dev_spk
    tr '[:lower:]' '[:upper:]' < $conf_dir/test_spk.list > $tmpdir/test_spk
    ls -d "$rootdir"/TRAIN/DR*/* | sed -e "s:^.*/::" > $tmpdir/train_spk
else
    tr '[:upper:]' '[:lower:]' < $conf_dir/dev_spk.list > $tmpdir/dev_spk
    tr '[:upper:]' '[:lower:]' < $conf_dir/test_spk.list > $tmpdir/test_spk
    ls -d "$rootdir"/train/dr*/* | sed -e "s:^.*/::" > $tmpdir/train_spk
fi
...
python utils/create-decode-graph.py \
    --use-silence \
    $langdir/phones.txt \
    > $langdir/phone_graph.txt || error_exit "Failed to create decode graph"
