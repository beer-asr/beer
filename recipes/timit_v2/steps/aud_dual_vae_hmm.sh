...
if [ ! -f $mdl_dir/0.mdl ]; then
    ...
    nspk=$(cat $data_train_dir/utt2spk | awk '{print $2}' | sort | uniq | wc -l)
    cat conf/aud_dual_vae_hmm/gmm.yml | sed s/\{size\}/$nspk/g \
        > $mdl_dir/gmm.yml
    ...
    python utils/gmm-create.py \
        --dim $aud_vae_hmm_latent_dim \
        $mdl_dir/gmm.yml \
        $mdl_dir/c_latent_model.mdl || exit 1
    ...
    vars="dim_in=$((2 * aud_vae_hmm_latent_dim)),width=$aud_vae_hmm_nnet_width,dim_out=$feadim"
    python utils/nnet-create.py \
        --set "$vars" \
    ...
    python utils/nnet-create.py \
        --set "$vars" \
        conf/aud_dual_vae_hmm/context_encoder.yml \
        $mdl_dir/c_encoder.mdl || exit_msg "Failed to create the VAE encoder"

    vars="dim_in=$aud_vae_hmm_latent_dim,width=$aud_vae_hmm_nnet_width,dim_out=$feadim"
    python utils/nnet-create.py \
        --set "$vars" \
        conf/aud_dual_vae_hmm/context_decoder.yml \
        $mdl_dir/c_decoder.mdl || exit_msg "Failed to create the VAE decoder"

    vars="dim_in=$aud_vae_hmm_latent_dim"
    python utils/nflow-create.py \
        --set "$vars" \
        conf/aud_dual_vae_hmm/context_normalizing_flow.yml \
        $mdl_dir/c_nflow.mdl || exit_msg "Failed to create the VAE norm. flow"

    python utils/dual-vae-hmm-create.py \
...
if [ ! -f $mdl_dir/final.mdl ];then
    ...
    while [ $((++iter)) -le $aud_vae_hmm_train_iters ]; do
        ...
        if echo $aud_vae_hmm_align_iters | grep -w $iter >/dev/null; then
            ...
            cmd="python utils/dual-vae-hmm-align.py \
                $mdl_dir/$mdl  $fea $tmpdir"
            utils/parallel/submit_parallel.sh \
                "$parallel_env" \
                "dual-vae-hmm-align-iter$iter" \
            ...
            python utils/dual-vae-hmm-set-decoding-graph.py \
                $mdl_dir/$mdl \
                $mdl_dir/decode_graph.pkl \
                $mdl_dir/phones_hmm.graphs \
                $mdl_dir/$mdl || exit
        ...
        cmd="python -u utils/dual-vae-hmm-train-with-alignments.py \
                --verbose \
