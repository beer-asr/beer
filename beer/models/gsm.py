...
class HierarchicalAffineTransform(AffineTransform):
    '''Affine transform (child) whose parameters are generated by another subspace model (parent/root_transform)'''
    @classmethod
    def create(cls, latent_prior, in_dim, out_dim, transform, cov_type='diagonal', latent_posterior=None):
        ...
        latent_posterior (``Model``): Posterior of the latent space. Will be created if not supplied'''
        if latent_posterior is None:
            latent_posterior = cls.new_latent_posteriors(transform, 1, cov_type)
        return cls(latent_prior, latent_posterior, in_dim, out_dim, transform)
    
    def __init__ (self, latent_prior, latent_posterior, in_dim, out_dim, transform):
    ...
    def new_latent_posteriors(transform, nposts, cov_type='diagonal'):
        'Create a set of `nposts` latent posteriors.'
        if cov_type not in ['full', 'diagonal']:
            raise UnknownCovarianceType('f{cov_type}')
        tensor = transform.bias.prior.params.mean
        dim = transform.in_dim
...
class HierarchicalGSM(Model):
    '''A placeholder model for elbo computation of a set of GSMs with HierarchicalAffineTransform transforms'''
    def __init__(self, shared_transform, latent_prior):
        super().__init__()
    ...
    def expected_log_likelihood(self, stats, latent_posts, univ_latent_nsamples=1,
        ...
        sub_elbos = []
        self.cache['lp_stats'] = 0
        for (model, sub_model), latent_post in zip(stats, latent_posts):
            llh = evidence_lower_bound(model, sub_model,
                                       latent_posts=latent_post,
                                       **kwargs)
            s_h = model.transform.latent_posterior.sample(univ_latent_nsamples)
            xent, _stats = _xentropy(s_h, self.latent_prior)
            ent = _entropy(s_h,  model.transform.latent_posterior)
            local_kl_div = xent - ent
            sub_elbos.append(llh.value - local_kl_div)
            self.cache['lp_stats'] += _stats
        return torch.stack(sub_elbos)
