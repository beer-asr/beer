...
from ..inference import objectives as obje
...
class HierarchicalGSM(Model):
    ...
    def __init__(self, shared_transform):
        super().__init__()
    ...
    def expected_log_likelihood(self, stats, latent_posts, univ_latent_nsamples=1,
        ...
        sub_elbos = [obje.evidence_lower_bound(model, submodel,
                                               latent_posts=latent_post,
                                               **kwargs)
                     for (model, submodel), latent_post in zip(stats, latent_posts)]
        sum_sub_elbos = sub_elbos[0]
        for _elbo in sub_elbos[1:]:
            sum_sub_elbos += _elbo
        self.cache['elbo_cache'] = sum_sub_elbos.__dict__
        self._bayesian_parameters = self.cache['elbo_cache']['_model_parameters']
        return sum_sub_elbos.value
         
    def kl_div_posterior_prior(self):
        # Compute intensive KLD
        return self.shared_transform.kl_div_posterior_prior()
    
    def accumulate(self, stats):
        return self.cache['elbo_cache']['_acc_stats']
    
    def bayesian_parameters(self):
        return self._bayesian_parameters
